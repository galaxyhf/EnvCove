...
O **EnvVault** é uma aplicação SaaS para armazenar, organizar e distribuir variáveis de ambiente com segurança.
...
- Download de secrets pelo terminal com `envvault pull`.
- Execução de processos com secrets em memória usando `envvault run`.
...
```bash
cd /Users/caiosilva/Developer/EnvVault
pnpm install
```
...
```text
postgresql://usuario:senha@ep-exemplo-pooler.regiao.aws.neon.tech/envvault?sslmode=require
```
...
```env
# Banco de dados
DATABASE_URL="postgresql://localhost/envvault"
...
### 3. Conectar um repositório ao EnvVault
...
Escolha o projeto, o ambiente e o arquivo de saída. Será criado um `.envvault.json` semelhante a:
...
```http
Authorization: Bearer ev_live_xxxxxxxxx
```
...
```bash
pnpm --filter @envvault/web start
```
...
```bash
pnpm --filter @envvault/web exec next build --webpack
```
...
### `ENVVAULT_MASTER_KEY is not configured`
...
### `ENVVAULT_MASTER_KEY must be 32 bytes encoded as base64`
...
```bash
pnpm --filter @envvault/web dev -- --port 3001
```
...
- Nunca reutilize `AUTH_SECRET` como `ENVVAULT_MASTER_KEY`.
