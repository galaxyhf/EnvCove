...
export const environmentSchema = projectSchema.pick({ name: true, slug: true, description: true });
...
export function parseEnv(content: string): Array<{ key: string; value: string }> {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    ...
    const parsed = secretSchema.pick({ key: true, value: true }).safeParse({ key, value });
    if (parsed.success) variables.push(parsed.data);
  }
  return variables;
}

export function serializeEnv(values: Array<{ key: string; value: string }>): string {
