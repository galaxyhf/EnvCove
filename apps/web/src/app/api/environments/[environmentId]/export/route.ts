...
export async function GET(
  ...
  try {
    ...
    await audit({
      ...
      metadata: { count: String(rows.length) },
    });
    const filename =
      new URL(request.url).searchParams.get("filename") ?? ".env";
    return new Response(
      serializeEnv(
        rows.map((row) => ({ key: row.key, value: decryptSecret(row) })),
      ),
      {
        headers: {
          "content-type": "text/plain; charset=utf-8",
          "content-disposition": `attachment; filename="${filename.replace(/[^.a-zA-Z0-9_-]/g, "") || ".env"}"`,
